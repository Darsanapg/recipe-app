...
const Home= () => {
    ...
    const getData=async()=>{
        try{
          ...
          const item='beef';
          const res=await fetch(`https://api.edamam.com/api/recipes/v2?type=public&q=beef&app_id=${APP_ID}&app_key=${APP_KEY}`,{
method:'GET',
headers: {
    ...
    useEffect(()=>{
      ...
      <Image
       ...
       />
                ...
                <Text style={{ fontWeight: 'bold', color: "black", fontSize:40, alignItems:'center' }}>BEEF RECIPE</Text>
                <TextInput
                type="text"
                name="searchBar"
                id="searchBar"
